...
func (p *Parser) Parse() error {
	//fmt.Println("buff:", string(p.buff))
...
func (p *Parser) parseTimestamp() (time.Time, error) {
	...
	index2 := 2
	index1 := 2
	index1 += bytes.Index(p.buff, []byte(":"))
	p.cursor = index1
	...
	if !found {
		// fix cisco offsets
		if bytes.Equal(p.buff[index1-1:index1], []byte{32}) {
			index2 += bytes.Index(p.buff[index1:], []byte(":"))
			index1 += index2
		}
		p.cursor = index1
		// end cisco fixes
...
func (p *Parser) parseTag() (string, error) {
	...
	from = bytes.Index(p.buff[p.cursor:], []byte{37}) + p.cursor - 1
	...
	for {
		...
		if endOfTag {
			if !found {
				if p.cursor <= len(p.buff) && from <= len(p.buff) && p.cursor > from {
					tag = p.buff[from:p.cursor]
				} else {
					tag = p.buff
				}
